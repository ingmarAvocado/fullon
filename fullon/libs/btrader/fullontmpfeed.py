...
class FullonSimFeed(FullonFeed):
    ...
    def _resample(self):
        if self.compression != 1 and self.feed.period.lower() == 'minutes':
            # Create a resampling rule based on the desired compression factor
            # Assuming self.compression is the number of minutes for the new timeframe
            resampling_rule = f'{self.compression}T'

            # Resample the OHLC data
            ohlc_resampled = self.dataframe.resample(resampling_rule).ohlc()

            # Flatten the multi-level index created by the ohlc() function
            ohlc_resampled.columns = ohlc_resampled.columns.droplevel()

            # Resample the volume data
            volume_resampled = self.dataframe['volume'].resample(resampling_rule).sum()

            # Combine the resampled OHLC and volume data into a single DataFrame
            self.dataframe = pandas.concat([ohlc_resampled, volume_resampled], axis=1)
        elif self.compression != 1 and self.feed.period.lower() in ['hours', 'days']:
            # Adjust the resampling rule for hours or days
    ...
    def _fetch_ohlcv(self):
        ...
        if rows:
            if self.noise and not (self.params.compression == 1 and self.feed.period.lower() == "minutes"):
                self.dataframe = self.params.mainfeed.dataframe
                self._resample()
                rows = self.dataframe.reset_index().values.tolist()
            else:
                self._save_to_df(rows=rows)
            if not pkl_exists:
